...
bool ngl_package_patch_appliable ( ngl_package_patch * patch, ngl_vm * vm ) {
  ( void ) patch;
  ( void ) vm;
  return true;
  }
...
ngl_error * ngl_package_relocate_obj (
    ngl_vm * vm,
    ngl_package_patch * patch,
    ngl_uint size_diff,
    ngl_uint patch_count,
    ngl_uint to_insert
  ) {
  ngl_uint id = patch->id;
  ngl_uint kind = patch->kind;
  ngl_obj * obj = NULL;
  ngl_globals_get_obj_from_ids (
      &vm->globals,
      kind,
      id,
      &obj );
  ...
  if ( obj == NULL ) {
    ...
    ngl_ret_on_err ( ngl_globals_set_obj_from_ids (
          &vm->globals,
          kind,
          id,
          obj ) );
...
ngl_error * ngl_package_patch_apply (
    ngl_vm * vm,
    ngl_package_patch * patch,
    ngl_uint count
  ) {
  ...
  for ( uint32_t i = 0; i < count; i++ ) {
    memcpy ( OFFSET ( OBJ_FIELDS_START ( obj ),
             ...
             OFFSET ( patch, sizeof ( ngl_package_patch ) ),
             patch->to_insert );
...
ngl_error * ngl_package_apply ( ngl_package * pkg, ngl_vm * vm ) {
  ...
  for ( uint32_t f = 0; f < fixups->count; f++ ) {
    ngl_obj * to_write;
    if ( ngl_globals_get_obj_from_ids (
          &vm->globals,
          fixup->kind,
          fixup->id,
          &to_write ) != ngl_ok ) {
...
ngl_error * ngl_vm_run_package (
  ...
  ngl_buffer * code;
  ngl_ret_on_err ( ngl_globals_get_obj_from_ids (
      &vm->globals,
      2,
      0,
      ( ngl_obj ** ) &code ) );
