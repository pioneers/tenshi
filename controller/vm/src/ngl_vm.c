...
static ngl_error * ngl_add_type (ngl_globals * globals, ngl_type * type) {
  ...
  ngl_ret_on_err (ngl_table_set (
      &globals->obj_table,
      ngl_val_pointer (type),
      ngl_val_pointer (array)));
  ngl_ret_on_err (ngl_array_push_back (
      &globals->type_id_to_type_ptr,
      ngl_val_pointer (type)));
  return ngl_ok;
  }

static ngl_error * ngl_init_core (ngl_vm * vm) {
