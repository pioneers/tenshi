...
#define SELECT_BUCKET(\
  ...
  do { \
    ...
    ngl_table_bucket * macro_the_bucket = \
        (macro_buckets) + ((hashed) & (macro_self)->mask); \
    if (!macro_the_bucket->hash) { \
      macro_when_missing_first(macro_the_bucket); \
    } else if (macro_self->interface->compare(macro_the_bucket->key, \
                                            macro_key) == 0) { \
      macro_when_first(macro_the_bucket); \
    } else { \
      while (macro_the_bucket->next) { \
        macro_the_bucket = macro_the_bucket->next; \
        if (macro_self->interface->compare(macro_the_bucket->key, \
                                           macro_key) == 0) { \
