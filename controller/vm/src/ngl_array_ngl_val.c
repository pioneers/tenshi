...
ngl_error * ngl_array_init (ngl_array * self, ngl_type * ngl_array_elem_type) {
  return ngl_array_init_to_size (self, ngl_array_elem_type, ngl_array_default_size);
  }

ngl_error * ngl_array_init_to_size (
    ngl_array * self,
...
ngl_array * ngl_array_new_to_size (ngl_type * ngl_array_elem_type, ngl_int num_elements) {
  ngl_array * self = ngl_alloc_simple (ngl_array, 1);
  if (! self) {
    return NULL;
    }
  if (ngl_array_init_to_size (self, ngl_array_elem_type, num_elements) != ngl_ok) {
      free (self);
      return NULL;
    }
  return self;
...
static bool ngl_array_looped (ngl_array * self) {
  return self->start > self->past_end;
  }


bool ngl_array_grow (ngl_array * self) {
  ...
  if (ngl_array_looped (self)) {
    ...
    memcpy ((void *) new_data,
             ...
             end_size * sizeof (ngl_val));
    memcpy ((void *) (new_data + self->start),
             (void *) self->data,
             self->past_end * sizeof (ngl_val));
...
ngl_array_iter ngl_array_start (ngl_array * parent) {
  return (ngl_array_iter) {.parent = parent, .index = 0  };
  }

ngl_array_iter ngl_array_end (ngl_array * parent) {
  return (ngl_array_iter) {.parent = parent, .index = -1  };
  }
...
ngl_val * ngl_array_at (ngl_array_iter * self, ngl_int rel_idx) {
  ngl_val * elem_ptr;
  ngl_error * e = ngl_array_index (self->parent, self->index + rel_idx, &elem_ptr);
  if (e == ngl_ok) {
    return elem_ptr;
    }
  else {
