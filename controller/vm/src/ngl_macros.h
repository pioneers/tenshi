...
#define ngl_init_alien(alien_type) \
  ...
  join_token(ngl_type_, alien_type)->ptr_of = \
      join_token(join_token(ngl_type_, alien_type), _ptr)
...
#define ngl_init_base(base_type) \
  ...
  join_token(ngl_type_, base_type)->ptr_of = \
      join_token(join_token(ngl_type_, base_type), _ptr);
...
((ngl_type_composite *) join_token(ngl_type_, composite_type))-> \
  ...
  join_token(ngl_type_, composite_type)->ptr_of = \
      join_token(join_token(ngl_type_, composite_type), _ptr);
...
#define ngl_init_ptr(base_type) \
  ...
  join_token(join_token(ngl_type_, base_type), _ptr)->header.type = \
      ngl_type_ngl_type_pointer; \
  join_token(join_token(ngl_type_, base_type), _ptr)->name = \
      ngl_str_lit(string_of_macro(join_token(base_type, _ptr))); \
  join_token(join_token(ngl_type_, base_type), _ptr)->size = \
      sizeof(base_type *); \
((ngl_type_pointer *) join_token(join_token(ngl_type_, base_type), \
                                 _ptr))->subtype = \
        ngl_type_to_ngl_type(base_type); \
  join_token(join_token(ngl_type_, base_type), _ptr_ptr) = \
        (ngl_type *) ngl_alloc_simple(ngl_type_pointer, 1); \
  if (join_token(ngl_type_, base_type) == NULL) { \
    ngl_alloc_error(); \
    } \
  join_token(join_token(ngl_type_, base_type), _ptr_ptr)->header.type = \
      ngl_type_ngl_type_pointer; \
  join_token(join_token(ngl_type_, base_type), _ptr_ptr)->ptr_of = NULL; \
  join_token(join_token(ngl_type_, base_type), _ptr_ptr)->name = \
      ngl_str_lit(string_of_macro(join_token(base_type, _ptr_ptr))); \
  join_token(join_token(ngl_type_, base_type), _ptr_ptr)->size = \
      sizeof(base_type **); \
((ngl_type_pointer *) join_token(join_token(ngl_type_, base_type), \
      _ptr_ptr))->subtype = \
      ngl_type_to_ngl_type(join_token(base_type, _ptr)); \
  join_token(join_token(ngl_type_, base_type), _ptr)->ptr_of = \
      join_token(join_token(ngl_type_, base_type), _ptr_ptr);
