...
#include <ngl_macros.h>
  ...
  #error Please define ngl_table_key_kind to one of integer, uinteger, floating, or pointer.
  #endif

#ifndef ngl_table_val_kind
  #error Please define ngl_table_val_kind to one of integer, uinteger, floating, or pointer.
  #endif
  ...
  #define ngl_table_name join_token (join_token (ngl_table_key_name, _to_), ngl_table_val_name)
  #endif

#define method(mname) join_token (join_token (ngl_table_, ngl_table_name), join_token (_, mname))

#define ngl_table_struct join_token (ngl_table_, ngl_table_name)

#define ngl_table_interface join_token (ngl_table_, join_token (ngl_table_name, _interface))
...
#define ngl_table_backcast_key(key) ((ngl_table_key_t) (key).ngl_table_key_kind)
#define ngl_table_backcast_val(val) ((ngl_table_val_t) (val).ngl_table_val_kind)
...
#define pointer 0xefaced
  ...
  #define ngl_table_cast_key(key) ((ngl_val) { .ngl_table_key_kind = (ngl_obj *) key })
#else
...
#define pointer 0xefaced
  ...
  #define ngl_table_cast_val(val) ((ngl_val) { .ngl_table_val_kind = (ngl_obj *) val })
#else
...
static ngl_table_struct * method (new) (void) {
  return (ngl_table_struct *) ngl_table_new (
      ngl_table_key_ngl_type,
      ngl_table_val_ngl_type,
      &ngl_table_interface);
  }

static ngl_error * method (init_to_size) (
    ngl_table_struct * self,
    size_t predicted_elems
) {
  return ngl_table_init_to_size (
      ngl_table_struct_to_table (self),
      ngl_table_key_ngl_type,
      ngl_table_val_ngl_type,
      predicted_elems,
      &ngl_table_interface);
...
static ngl_error * method (init) (
    ngl_table_struct * self
) {
  return ngl_table_init (
      ngl_table_struct_to_table (self),
      ngl_table_key_ngl_type,
      ngl_table_val_ngl_type,
      &ngl_table_interface);
...
static ngl_error * method (get) (ngl_table_struct * self, ngl_table_key_t key, ngl_table_val_t * dst) {
  return ngl_table_get_hash (
      ngl_table_struct_to_table (self),
...
static ngl_error * method (get_or_add) (
    ngl_table_struct * self,
    ngl_table_key_t key,
    ngl_table_val_t * dst,
    ngl_call * on_missing
...
static ngl_error * method (get_default) (
    ngl_table_struct * self,
    ngl_table_key_t key,
    ngl_table_val_t * dst,
