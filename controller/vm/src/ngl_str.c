...
ngl_str ngl_str_from_static ( const char * str ) {
  ngl_str out;
...
#ifndef ANGELIC_MICRO
static long int ngl_get_file_size ( FILE * file ) {
...
ngl_str ngl_str_from_file ( FILE * file ) {
  ...
  if ( file == NULL ) {
    ...
    long int to_read = ngl_get_file_size ( file );
...
size_t ngl_str_code_length ( const ngl_str str ) {
  size_t count = 0;
...
void ngl_str_println ( ngl_str str ) {
  ngl_str_print ( str );
  printf ( "\n" );
  }
...
ngl_hash ngl_str_hash ( ngl_str self ) {
...
void ngl_str_init_alloc ( ngl_str * self, const char * start, const char * past_end, enum ngl_str_alloc_type alloc_type ) {
  if ( ( ! past_end ) && start ) {
    past_end = start;
    while (*past_end) {
      past_end++;
...
ngl_str * ngl_str_new_alloc ( const char * start, const char * past_end, enum ngl_str_alloc_type alloc_type ) {
...
ngl_str * ngl_str_from_llong ( long long input ) {
