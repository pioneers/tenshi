...
inline static void siphash_round (sip_hash_state * s, unsigned int rounds) {
  while (rounds-- != 0) {
    s->v[0] += s->v[1];
    SIP_ROT64 (s->v[1], 13);
    s->v[1] ^= s->v[0];
...
inline uint64_t siphash_24 (const ngl_siphash_key key, const uint8_t * msg, const size_t len) {
  sip_hash_state s;
...
ngl_siphash_key ngl_hash_key = { { 0xe8f35937acefffaa, 0x331e89da1849403f } };
