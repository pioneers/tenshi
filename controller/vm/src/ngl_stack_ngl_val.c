...
ngl_stack_bucket * ngl_stack_bucket_alloc (ngl_uint size, ngl_stack_bucket * prev) {
...
static ngl_stack_bucket * ngl_stack_get_bucket (ngl_stack * self, ngl_uint bucket) {
...
ngl_error * ngl_stack_set_size (ngl_stack * self, ngl_uint bucket, ngl_uint size) {
  if (bucket == 0) {
    ngl_stack_bucket * new_bucket = ngl_stack_bucket_alloc (size, self->top->prev);
    if (new_bucket == NULL) {
      return &ngl_out_of_memory;
      }
