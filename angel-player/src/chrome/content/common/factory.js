...
var factory_prototype = {
  ...
  get_alignment : function get_alignment ( type ) {
    ...
    if ( actual_type === undefined ) {
      throw 'Could not get alignment of type ' + type;
      }
    else {
      type = actual_type;
      }
    var base_size = this._get_size_base ( type );
    if ( base_size !== undefined ) {
      ...
      if ( res > 0 ) {
        ...
        // Handles, among other things, structs with no slots.
        return 1;
        }
      }
    else {
