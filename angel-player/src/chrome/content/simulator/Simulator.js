...
Simulator.prototype.loadMap = function(mapId)
{
    ...
    {
        var temp = map.objects[i];

        if(temp.type == "BOX")
        {
            this.createBox(temp.width, temp.height, temp.depth, temp.mass, parseInt(temp.color),
    ...
    for(var i = 0; i < map.robot.length; i++)
    {
        var temp = map.robot[i];
...
Simulator.prototype.loadRobot = function(robotJson, position)
{
    ...
    for(var i = 0; i < data.chassi.length; i++)
    {
        ...
        if(temp.type == "BOX")
        {
            robot.addShape(new Ammo.btBoxShape(new Ammo.btVector3(temp.size[0]/2, temp.size[1]/2, temp.size[2]/2)),
                           ...
                           temp.mass,
                           createBoxMesh(temp.size[0], temp.size[1], temp.size[2], parseInt(temp.color), this.scene));
    ...
    {
        var temp = data.motors[i];
        var motor = new Motor(this, temp.boxSize[0], temp.boxSize[1], temp.boxSize[2], temp.boxMass, parseInt(temp.boxColor),
                              temp.wheelRad, temp.wheelHeight, temp.wheelMass, parseInt(temp.wheelColor),
...
Simulator.prototype.render = function()
{
    ...
    else
    {
        ...
        for(bot in this.robots)
        {
            bot.simulate(); // check version, update motors as necessary, write new sensor to master
            bot.updateSensors(); // writes sensory data to master object
        }
